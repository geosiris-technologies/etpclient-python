...
class ETPDataBridge:
    ...
    async def handle_request(self, etp_req, client_info: typing.Union[None, ClientInfo] = None):
        ...
        try:
            if isinstance(etp_req, GetResources):
                ...
                return await self.handle_delete_data_objects(client_info, etp_req)
            elif isinstance(etp_req, GetDeletedResources):
                return await self.handle_get_deleted_resources(client_info, etp_req)
            elif isinstance(etp_req, GetSupportedTypes):
                return await self.handle_get_supported_types(client_info, etp_req)
            elif isinstance(etp_req, DeleteDataspaces):
                return await self.handle_delete_dataspaces(client_info, etp_req)
                ...
                print("#ETPDataBridge can not handle request of type ", type(etp_req))
                return None, None
    ...
    async def handle_put_data_objects(self, client_info: typing.Union[None, ClientInfo], put_data_objects: PutDataObjects) -> (PutDataObjectsResponse, ETPError):
        ...
        for uri in put_data_objects.data_objects:
            ...
            response.append(await ingest_xml(file_content= data_obj.data.decode(), file_name=parsed_uri.uuid+".xml", data_manager=data_manager))

        put_response_map = {}
        for uri in response:
            put_response_map[uri] = PutResponse(created_contained_object_uris=[uri])

        return PutDataObjectsResponse(success=put_response_map), None

    async def handle_get_deleted_resources(self, client_info: typing.Union[None, ClientInfo], get_deleted_resources: GetDeletedResources) -> (GetDeletedResourcesResponse, ETPError):
        ...
        deleted_resources = []
        for res in get_deleted_objects(dm=data_manager, time_filter=deleteTimeFilter,dataObjectTypes=dataObjectTypes):
            deleted_resources.append(DeletedResource(uri=res["uri"],
                                                    data_object_type=res["dataObjectType"],
                                                    deleted_time=int(res["deletedTime"]),
                                                    custom_data={},))

        return GetDeletedResourcesResponse(deleted_resources=deleted_resources), None

    async def handle_delete_data_objects(self, client_info: typing.Union[None, ClientInfo], delete_data_objects: DeleteDataObjects) -> (DeleteDataObjectsResponse, ETPError):
        ...
        for uri_id in uris:
            ...
            print(client_info.ip, ":", "#DeleteDataObjects --> remove on map : ", mapUuidToUri)
        ...
        for idx in range(0, len(uri_deleted_objects)):
            if uri_deleted_objects[idx]:
                print(client_info.ip, ":", "#DeleteDataObjects --> getting uri for " + uri_deleted_objects[idx])
                # uri = mapUuidToUri[]
                delete_response_map[uri] = ArrayOfString(values=[uri_deleted_objects[idx]])


        return DeleteDataObjectsResponse(deleted_uris=delete_response_map), None

    async def handle_get_data_objects(self, client_info: typing.Union[None, ClientInfo], get_data_objects: GetDataObjects) -> (GetDataObjectsResponse, ETPError):
        ...
        for uri_id in uris:
            ...
            if data and data['Body']:
                ...
                version = data_manager.xml_manager.get_object_last_versions(uuid)
                obj_type = data_manager.xml_manager.get_object_tags(uuid, version)["obj_type"]  # TODO : faire des varibles gloables pour les tags
                if "." in obj_type:
                ...
                graph_obj = data_manager.graph_manager.get_document(collection_name=obj_type, document_key=uuid+"."+version)
                ...
                if get_data_objects.format == "json":
                    data_obj_res_map[uuid] = DataObject(blob_id=to_Uuid(pyUUID.UUID('{'+uuid+'}')),
                                                        format="json",
    ...
    async def handle_get_resources(self, client_info: typing.Union[None, ClientInfo], etp_get_resources: GetResources) -> (GetResourcesResponse, ETPError):
        ...
        try:
            ...
            if(not uri or uri.lower() == "eml:///"):  # pas de filtre, on garde tout                
                ...
                req_answer = request_objects(dm=data_manager, type_filter=dataObjectTypes)
                ...
                data_manager = dataspaces_handler.get_data_manager(dataspace_name)
                print(client_info.ip, ":", "URI ", parsed_context_uri, " \nfrom ", uri)

                if isinstance(parsed_context_uri, DataObjectURI):
                    obj_type = refactor_object_type(parsed_context_uri.object_type)
                    obj_uuid = parsed_context_uri.uuid
                    include_self = "self" in str(etp_get_resources.scope).lower()
                    up = "sources" not in str(etp_get_resources.scope).lower()
                    down = "targets" not in str(etp_get_resources.scope).lower()
                    depth = etp_get_resources.context.depth

                    version = data_manager.xml_manager.get_object_last_versions(obj_uuid)
                    # obj_type = data_manager.xml_manager.get_object_tags(uuid, version)["obj_type"] 
                    req_answer = get_firp_connected_objects(dm=data_manager, uuid=obj_uuid, obj_type= obj_type, version=version, depth=depth, outcomming=up, incoming=down, include_self=include_self)
                    print(client_info.ip, ":", "get_firp_connected_objects", req_answer,"\ndataspace_name ", dataspace_name, "\ndata_manager", data_manager, 
                        "obj_type", obj_type, "\n",
                        "obj_uuid", obj_uuid, "\n",
                    ...
                    print(client_info.ip, ":", "@ETPDataBridge#handle_get_resources : Unkown URI type")
        ...
        return self._get_get_resources_response_from_json(req_answer)  # _get_get_resources_response_from_json retourne deja l'erreur

    async def handle_get_supported_types(self, client_info: typing.Union[None, ClientInfo], get_supported_types: GetSupportedTypes) -> GetSupportedTypesResponse:
        ...
        if uri == "eml:///":
            data_manager = dataspaces_handler.get_data_manager()
            obj_map = data_manager.xml_manager.list_bucket_objects_count_versions_and_tags()
            
            for obj_uuid in obj_map:
                obj = obj_map[obj_uuid]
                supported_type_list.append(SupportedType(data_object_type=obj["obj_type"], object_count= (obj["count"] if count else None) ))
            ...
            if isinstance(parsed_context_uri, DataObjectURI):
                ...
                obj_last_version = get_last_object_version(dm=data_manager, obj_type=obj_type, uuid=obj_uuid)
                if obj_last_version:
                    obj_id = (obj_type if "Obj_" not in obj_type else obj_type[4:]) + "/" + obj_uuid + "." + obj_last_version
                    obj_map_count = count_distinct_accessible_objects_per_type(dm=data_manager, start_obj_id=obj_id, depth=-1, outcomming=out, incoming=incom, include_self=include_self)
                    for obj in obj_map_count:
                        print(client_info.ip, ":", "OBJ ", obj)
                        if not obj["isVoider"] or get_supported_types.return_empty_types:
                            obj_count = obj["count"]
                            if obj["isVoider"]:
                                obj_count = 0
                            supported_type_list.append(SupportedType(data_object_type=obj["type"], object_count= obj_count if count else None ))
        ...
        if get_supported_types.return_empty_types:
            for obj_type in ETP_SUPPORTED_TYPES:
                if obj_type not in obj_type_list:
                    supported_type_list.append(SupportedType(data_object_type=obj_type, object_count=(0 if count else None) ))


        return GetSupportedTypesResponse(supported_types=supported_type_list), None


    async def handle_delete_dataspaces(self, client_info: typing.Union[None, ClientInfo], delete_dataspaces: DeleteDataspaces)  -> (DeleteDataspacesResponse, ETPError):
        dataspace_to_remove = []
        for d_uri in delete_dataspaces.uris.values():
            ...
            try:
                ...
                if not DataspaceDB.get_dataspace(dataspace_name=dataspace_name):
                    return None, NotFoundError()
        ...
        for d_name in dataspace_to_remove:
            has_been_deleted = DataspaceDB.delete_dataspace(dataspace_name=d_name)
    ...
    async def handle_get_dataspaces(self, client_info: typing.Union[None, ClientInfo], get_dataspaces: GetDataspaces)  -> (GetDataspacesResponse, ETPError):
        dataspaces = DataspaceDB.get_dataspaces()

        dataspaces_res = []
        for d in dataspaces:
            # TODO : Store changes in dataspaces to have the "last_changed"
            ds = Dataspace(uri="eml:///dataspace('"+d.name+"')", last_changed=0, path="", custom_data={})
            dataspaces_res.append(ds)
        
        return GetDataspacesResponse(dataspaces=dataspaces_res), None

    async def handle_put_dataspaces(self, client_info: typing.Union[None, ClientInfo], put_dataspaces: PutDataspaces)  -> (PutDataspacesResponse, ETPError):
        ...
        for d in put_dataspaces.dataspaces.values():
            ...
            if not DataspaceDB.get_dataspace(dataspace_name):
                ...
                default_dataspace.data_deleted.db_name = dataspace_name + "_deleted"
        ...
        for r_att, r_att_type in get_class_attributes(Resource):
            ...
            for etp_obj_att in rdms_obj:
                ...
                if r_att_lw == etp_obj_att.lower():
                    find = True
                    if (r_att_lw == "lastchanged"
                        or r_att_lw == "lastmodified"
                        or r_att_lw == "storelastwrite"):  
                        # si on doit convertir une date en entier
                        map_resource[r_att] = self._convert_date_str_to_int(rdms_obj[etp_obj_att])
                    else:
            ...
            if not find or map_resource[r_att] == "":
                ...
                if (r_att_type.startswith("typing.List")
    ...
    def _get_get_resources_response_from_json(self, json_data_list) -> (GetResourcesResponse, ETPError):
        resource_list = []
        ...
        return GetResourcesResponse.parse_obj({"resources": resource_list}), None
