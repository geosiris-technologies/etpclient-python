...
class HSDSBridge:
    ...
    def handle_metadata(self, etp_req: GetDataArrayMetadata):
        ...
        for key, value in dataArrays.items():
            ...
            try:
                ...
                fin = h5pyd.File(self².domain, mode='r', endpoint=self.endpoint, username=self.username, password=self.password, bucket=self.bucket, use_cache=True)
            except IOError as ioe:
                if ioe.errno == 403:
                    logging.error("No read access to domain: {}".format(self.domain))
                elif ioe.errno == 404:
                    logging.error("Domain: {} not found : {}".format(self.domain, ioe))
                elif ioe.errno == 410:
                    logging.error("Domain: {} has been recently deleted".format(self.domain))
                else:
                    logging.error("Error opening domain {}: {}".format(self.domain, ioe))

            if fin :
                dataset = fin[value.path_in_resource]
                if dataset:
                    arrayMetadata[key] = DataArrayMetadata(dimensions=list(dataset.shape), array_type=AnyArrayType.ARRAY_OF_FLOAT)
    ...
    def send_get_data_arrays(self, get_data_arrays: GetDataArrays) -> (GetDataArraysResponse, ETPError):
        ...
        for key, value in dataArrays.items():
            ...
            if fin:        
                ...
                data = AnyArray(item=ArrayOfFloat(values=list(dataset[()].flatten())))
                dataArraysResponse[key] = DataArray(dimensions=list(dataset.shape), data=data)
            
        return GetDataArraysResponse(data_arrays=dataArraysResponse), None  
        #{"test": DataArray(dimensions=[1],data=AnyArray(item=b"000000"))}

    def send_get_data_subarrays(self, get_data_arrays: GetDataSubarrays) -> (GetDataSubarraysResponse, ETPError):
        ...
        for key, value in dataSubarrays.items():
            ...
            if fin:
                ...
                dataSubarraysResponse[key] = DataArray(dimensions=list(subdataset.shape), data=data)

        return GetDataSubarraysResponse(data_subarrays=dataSubarraysResponse), None  
            
    def send_put_data_arrays(self, put_data_arrays: PutDataArrays) -> (PutDataArraysResponse, ETPError):
        ...
        for key, value in dataArrays.items():
            ...
            try:
                ...
                fin = h5pyd.File(self.domain, mode='a', endpoint=self.endpoint, username=self.username, password=self.password, bucket=self.bucket, use_cache=True)
            except IOError as ioe:
                if ioe.errno == 403:
                    logging.error("No read access to domain: {}".format(self.domain))
                elif ioe.errno == 404:
            ...
            if fin:
                try:
                    eda = fin.create_dataset(name=path,shape=da.dimensions,data=da.data.item.values)
                    print(eda[()])
                    # fin[path] = eda
